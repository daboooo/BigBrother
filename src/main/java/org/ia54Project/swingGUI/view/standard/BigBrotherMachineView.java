...
public class BigBrotherMachineView extends BigBrotherDetailView {
	private static final long serialVersionUID = -7743458183614453792L;
	private JTable tableDesc;
	private JTable tableValues;

	public BigBrotherMachineView() {
		initialize();
	}
	
	public BigBrotherMachineView(MachineModel model) {
		initialize();
		setModel(model);
	}
	
	public void setModel(MachineModel model) {
		tableDesc.getModel().setValueAt("Hostname :", 0, 0);
		tableDesc.getModel().setValueAt("IP :"		, 1, 0);
		tableValues.getModel().setValueAt(model.getName()  	, 0, 0);
		tableValues.getModel().setValueAt(model.getIp()		, 1, 0);
	...
	private void initialize() {
		...
		JLabel lblRoleDetails = new JLabel("Machine Details");
